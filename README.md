# YN ZERNIA PIXIV Downloader 6.5

YN ZERNIA PIXIV 다운로더 6.5를 사용해 주셔서 감사합니다. 이 프로그램의 주요 기능과 사용법, 주의사항을 안내합니다.
...
1. GitHub 저장소에서 새 Release를 만들고 태그를 `v6.5.0`처럼 현재 프로그램보다 높은 버전으로 지정합니다.
2. 빌드한 `YN Zernia Pixiv Downloader 6.5.exe` 파일을 Release asset으로 첨부합니다.
...
Thank you for using YN ZERNIA PIXIV Downloader 6.5. Below is a guide to its main features, usage, and precautions.
...
1. Create a GitHub Release with a version tag newer than the app, such as `v6.5.0`.
2. Attach the built `YN Zernia Pixiv Downloader 6.5.exe` as a Release asset.
